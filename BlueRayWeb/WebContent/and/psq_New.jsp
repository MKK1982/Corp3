		...
		if (request.getParameter("type") == null) {
			...
			if (no == 1) {
				...
				try {
					...
					String QueryString = "SELECT uname,pword from HH where DEVICEID='"
							+ devId + "'";
				...
				try {
					...
					conn2 = DriverManager.getConnection(
							"jdbc:oracle:thin:@localhost:1521:xe", Uname,
							Pword);
					...
					while (rs2.next()) {
						...
						//if(i<=205)
						{
							...
							ShrtName = rs2.getString("Name")
									.replace(',', ' ').replace('^', ' ');

							output = output + rs2.getLong("Account_No")
									+ "^" + ShrtName + "^" + "-" + "^"
									+ rs2.getFloat("Current_Balance") + ",";
				...
				try {
					...
					//String query = "select * from DS_table";
					String query = "SELECT  * FROM    ( SELECT  q.*, rownum rn FROM    ( SELECT  * FROM     DS_table ORDER BY Account_No  ) q  ) WHERE   rn BETWEEN 1801 AND 1950";
					rs2 = statement2.executeQuery(query);
				...
				//accno
				float ClAmt = Float.parseFloat(request
						.getParameter("Collected"));
				//old int Acc_No=Integer.parseInt(request.getParameter("Accountno"));
				//old  int accno =Acc_No;
				//new
				long Acc_No = Long.parseLong(request
				...
				try {
					...
					String query = "update DS_table set Collected_Amt='"
							+ collectAmt + "',Collected_Date=to_date('"
							+ collectDt + "','yyyyMMdd'),device_id='"
							+ devId + "',UserId='" + userId
							+ "',timeofentry='" + Tm
				...
				try {
					...
					while (rs2.next()) {
						...
						output = output + rs2.getInt("Account_No") + "^"
								+ rs2.getString("Transaction_date") + "^"
								+ rs2.getFloat("Transaction_amount") + "^"
				...
				try {
					...
					// sql query to retrieve values from the secified table.
					String QueryString = "SELECT distinct Bcode from HH where DEVICEID='"
							+ devId + "'";
				...
				try {
					...
					String QueryString = "Select Branch_Name||','||CASH||','||SK_CASH||',' ||CASH_HUB from Daily_CB where CB_Date='"+ Dt + "' and BRANCH_CODE !=206 order by Branch_Code";
					//out.println(QueryString);
					...
					String QueryString2 ="Select 'Total'||','||sum(CASH)||','||sum(SK_CASH)||',' ||sum(CASH_HUB) from Daily_CB where CB_Date='"+ Dt + "' and BRANCH_CODE !=206";
				...
				//	String QueryString = "Select Branch_Name||','||Abs(GET_GL_CB2_VIEW(Branch_Code,29000,'"+ Dt + "'))||','||Abs(GET_GL_CB2_VIEW(Branch_Code,29006,'"+ Dt + "'))||',0' as Cash from Branch_Master order by Branch_Code";
					...
					//05-06-2023
					//String QueryString = " Select Branch_Name||','||Abs(GET_GL_CB2_VIEW(Branch_Code,29000,'"+ Dt + "'))||','||Abs(GET_GL_CB2_VIEW(Branch_Code,29006,'"+ Dt + "'))||',' ||Abs(GET_GL_CB2_VIEW(Branch_Code,(select gl_code from cash_hub_detail where bcode=Branch_Code),'"+ Dt + "')) as Cash from Branch_Master order by Branch_Code";
					String QueryString = "Select Branch_Name,Abs(GET_GL_CB2_VIEW(Branch_Code,29000,'"+ Dt + "')) as Cash,Abs(GET_GL_CB2_VIEW(Branch_Code,29006,'"+ Dt + "')) as SK_Cash,nvl(Abs(GET_GL_CB2_VIEW(Branch_Code,gl.gl_code,'"+ Dt + "')),0) as Cash_Hub from Branch_Master left join (select gl_code,bcode from cash_hub_detail where gl_code !=0)gl on Branch_Code=gl.bcode order by Branch_Code";
					//out.println(QueryString);
					...
					while (rs.next()) {
			                ...
			                obj.put("Cash",rs.getString(2));
			                obj.put("Cash_SK",rs.getString(3));
			                obj.put("Cash_Hub",rs.getString(4));
			               // array.add(obj);
			                array.add(obj);
					}
					...
					//String QueryString2 = "Select 'Total'||','||Abs(GET_GL_CB2_VIEW('%',29000,'"+ Dt + "'))||','||Abs(GET_GL_CB2_VIEW('%',29006,'"+ Dt + "'))||',0' as Cash from dual";
					//05-06-2023
					String QueryString2 ="Select 'Total' as Branch_Name,Abs(GET_GL_CB2_VIEW('%',29000,'"+ Dt + "')) as Cash,Abs(GET_GL_CB2_VIEW('%',29006,'"+ Dt + "')) as SK_Cash,(select sum(Abs(GET_GL_CB2_VIEW(bcode, gl_code,'"+ Dt + "'))) from cash_hub_detail WHERE gl_code!=0 ) as Cash_Hub FROM DUAL";

					rs2 = statement2.executeQuery(QueryString2);
					while (rs2.next()) {
						JSONObject obj = new JSONObject();
		                obj.put("Branch",rs2.getString(1));
		                obj.put("Cash",rs2.getString(2));
		                obj.put("Cash_SK",rs2.getString(3));
		                obj.put("Cash_Hub",rs2.getString(4));
		               // array.add(obj);
		                array.add(obj);
			...
			else if (no == 126) 
			{
				...
				try {
					...
					String QueryString = "Select Short_Name as Branch_Name,count(*) as Count,Round(sum(Gross_Weight),2) as Gross_Weight,sum(-1*Closing_Balance) as Os from JL_Master_View a left join Branch_Master b on a.Branch_Code=b.Branch_Code where Account_Status='A' group by a.Branch_Code,Short_Name order by a.Branch_Code";
					//out.println(QueryString);
					...
					statement2 = conn1.createStatement();
					String QueryString2 ="Select 'Total' as Branch_Name,count(*) as Count,Round(sum(Gross_Weight),2) as Gross_Weight,sum(-1*Closing_Balance) as Os from JL_Master_View where Account_Status='A'";
				...
				try {
					...
					statement = conn1.createStatement();
                             ...
                             String numberOnly= mon.replaceAll("[^0-9]", "");
                       int n=Integer.parseInt(numberOnly.trim());

//Log------------------------------------
		PreparedStatement p=null;
		
		  String logQuery="insert into APP_OPERATION_LOG(did,mon,type,no) values('"+Dt+"','"+mon+"','JL OS','"+no+"')";
...
//-----------------------------------------------		
					...
					String QueryString = "Select Branch_Code,Branch_Name,count(*) as cnt,sum(Os+Interest) as Os from JL_OD_LIST_MOB where OS_DATE='"+ Dt + "' and NVL(case when Period_Months=180 then 6 else case when Period_Months=90 then 3 else Period_Months End End,0) like '"+n+"%' group by Branch_Code,Branch_Name order by Branch_Code";
					//out.println(QueryString);
					...
					statement2 = conn1.createStatement();
					String QueryString2 ="Select 'Total' as Branch_Name,count(*) as cnt,sum(Os+Interest) as Os from JL_OD_LIST_MOB where OS_DATE='"+ Dt + "' and NVL(case when Period_Months=180 then 6 else case when Period_Months=90 then 3 else Period_Months End End,0) like '"+mon+"%'";
				...
				String devId = request.getParameter("did");//Request.QueryString["did"].ToString();
				String CDate = request.getParameter("Dt1");
				String Dt = CDate;//CDate.substring(4,2);
				//out.println("Date is= <b>" + Dt + "</b>");
				JSONArray array=new JSONArray();
				Connection conn1 = null;
				...
				String QueryString="select to_char(created_date,'HH:mi'),to_char(created_date,'AM') FROM daily_cb where CB_Date='"+Dt+"' and branch_code='201'";
					...
					while (rs.next())
					{
						...
						Be +=rs.getString(1)+" "+rs.getString(2);
						obj.put("Time",Be);
						array.add(obj);
					}
					//conn1.close();
...
//out.println(pwd);
	...
	//Log------------------------------------
				PreparedStatement p=null;
				  ...
				  String logQuery="insert into APP_OPERATION_LOG(mon,type,no,cid) values('"+Dt+"','"+Dt+"','"+Dt+"','"+Dt+"')";
				  p =conn1.prepareStatement(logQuery);
				  try{
					p.execute();
				  }catch(SQLException e){
...
//----------------------------------------------------------------------------------------------------
					...
					String QueryString = "Select Branch_Name,nvl(CASH,0),nvl(SK_CASH,0),nvl(CASH_HUB,0) from Daily_CB where CB_Date='"+ Dt + "' and BRANCH_CODE !=206 order by Branch_Code";
					//String QueryString = "Select Branch_Name,CASH,SK_CASH,CASH_HUB from Daily_CB2 where CB_Date='13/02/2024' and BRANCH_CODE !=206 order by Branch_Code";
...
//out.println(QueryString);
					...
					statement2 = conn1.createStatement();
					String QueryString2 ="Select 'Total' as total,sum(CASH),sum(SK_CASH),sum(CASH_HUB) from Daily_CB where CB_Date='"+ Dt + "' and BRANCH_CODE !=206";
					rs2 = statement2.executeQuery(QueryString2);
					while (rs2.next()) {
							    JSONObject obj = new JSONObject();
			                obj.put("Branch",rs2.getString(1));
